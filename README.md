...
4. Copiar `.env.example` como `.env` y pegar la llave de Anthropic:
   ```
   ANTHROPIC_API_KEY=sk-ant-...
   ```
   Solo hace falta para los dos botones de IA del generador.
5. Arrancar:
   ```
   npm start
   ```
...
public/
  ...
  konekt-config.example.js      plantilla (copiar como konekt-config.js)
...
| ------ | --------------- | --------------------------------------------------- |
| `POST` | `/api/extract`  | Texto libre → datos estructurados del proyecto      |
| `POST` | `/api/redactar` | Datos confirmados → contenido redactado de propuesta |
| `GET`  | `/api/health`   | Diagnóstico: modelo activo y si hay llave cargada    |

## Advertencias para esta etapa

- **No publiques este servidor en internet todavía.** Los endpoints `/api/*` no
  tienen autenticación de usuario: cualquiera con la URL podría consumir el
  presupuesto de Anthropic. Hay un límite básico por IP (40 peticiones cada 5
  minutos) como seguro mínimo, pero no sustituye autenticación real.
